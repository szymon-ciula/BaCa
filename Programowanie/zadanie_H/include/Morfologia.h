...
struct Inwersja : public Przeksztalcenie
{
    ...
    {
        for_each_piksel( bitmapa,
                         [](unsigned x, unsigned y, unsigned z, std::vector<Piksel>&, Bitmapa& bm, fun_warunek w) { bm(x,y,z) = !bm(x,y,z); },
                         nullptr,
                         nullptr );
    }
};
struct Erozja : public Przeksztalcenie
{
    void przeksztalc(Bitmapa& bitmapa) override
    {
        for_each_piksel( bitmapa,
                         dodaj_piksel,
                         [](unsigned x, unsigned y, unsigned z, const Bitmapa& bm) { return bm(x,y,z)==false; },
                         [](unsigned x, unsigned y, unsigned z, Bitmapa& bm) { bm(x,y,z) = false; }  );
    }
};
struct Dylatacja : public Przeksztalcenie
{
    void przeksztalc(Bitmapa& bitmapa) override
    {
        for_each_piksel( bitmapa,
                         dodaj_piksel,
                         [](unsigned x, unsigned y, unsigned z, const Bitmapa& bm) { return bm(x,y,z)==true; },
                         [](unsigned x, unsigned y, unsigned z, Bitmapa& bm) { bm(x,y,z) = true; }  );
    }
};
struct Zerowanie : public Przeksztalcenie
{
    ...
    {
        for_each_piksel( bitmapa,
                         [](unsigned x, unsigned y, unsigned z, std::vector<Piksel>&, Bitmapa& bm, fun_warunek w) { bm(x,y,z) = false; },
                         nullptr,
                         nullptr );
    }
};
struct Usrednianie : public Przeksztalcenie
{
    void przeksztalc(Bitmapa& bitmapa) override
    {
        for_each_piksel( bitmapa,
                         usredniany_piksel,
                         nullptr,
                         [](unsigned x, unsigned y, unsigned z, Bitmapa& bm) { bm(x,y,z) = !bm(x,y,z); }  );
    }
private:
    fun_dzialanie_warunek usredniany_piksel =
    [](unsigned x, unsigned y, unsigned z, std::vector<Piksel>& v, Bitmapa& bm, fun_warunek warunek)
    {
        unsigned short i = 0;
